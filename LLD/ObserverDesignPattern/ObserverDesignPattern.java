...
class YTnotification implements IChannel {
    private List<ISubscriber> subscribers;
    private String name;
    private String latestVideo;

    public YTnotification(String name) {
        this.name = name;
    ...
    public void subscribe(ISubscriber subscriber) { 
        if (!subscribers.contains(subscriber)) {  
            subscribers.add(subscriber);  // Add subscriber if not already subscribed
        }
    }
    ...
    public void notifySubscribers() {
        for (ISubscriber sub : subscribers) {
            sub.update();  // Notify each subscriber of the new video
        }
    }
...
class Subscriber implements ISubscriber {
    private String name;
    private YTnotification channel;

    public Subscriber(String name, YTnotification channel) {
        this.name = name;
...
public class ObserverDesignPattern {
    public static void main(String[] args) {
        ...
        YTnotification channel = new YTnotification("CoderArmy");
